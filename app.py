...
def home():
    ...
    if 'loggedin' in session:
        ...
        # If the user is a student, render the student home page
        else:
...
def create():
    ...
    if request.method == 'POST':
        ...
        cur.execute('SELECT * FROM Student WHERE regNo = %s', (username,))
        user_exists = cur.fetchone()

        # If the user exists, flash a message and redirect to the home page
        if user_exists:
...
def applied(id):
    ...
    try:
        cur.execute('INSERT INTO applied (regno, companies) VALUES (%s, %s)', (session['username'], comps))
        conn.commit()
    except Exception as e:
        conn.rollback()
